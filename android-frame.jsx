...
function AndroidDevice({
  ...
  return (
    <div style={{
      ...
      {false && title === undefined && <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, pointerEvents: 'none' }}>
        <AndroidStatusBar dark={dark} />
      </div>}
      {title !== undefined && <>
        <AndroidStatusBar dark={dark} />
        <AndroidAppBar title={title} large={large} />
      </>}
      {keyboard && <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10 }}><AndroidKeyboard /></div>}
    </div>
  );
}
